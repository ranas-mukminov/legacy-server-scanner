...
from ..model import LAN, FirewallAction, Policy, VPNType, WANType
...
class RouterOSBackend:
    ...
    def _render_wan(self, policy: Policy) -> List[str]:
        ...
        if wan.type == WANType.PPPOE:
            ...
            lines.append("/interface pppoe-client")
    ...
    def _render_wifi(self, wifi, policy: Policy) -> List[str]:
        ...
        lines.append("# Note: Adjust interface name based on your hardware")
        lines.append("/interface wireless")
        lines.append("set [ find default-name=wlan1 ] \\")
        lines.append(f"    ssid=\"{wifi.ssid}\" \\")
        lines.append(f"    mode={wifi.mode.value} \\")

        if wifi.security.encryption.value != "open":
            lines.append("    security-profile=default \\")
            lines.append("    # Configure security profile separately")

        if wifi.hidden:
            lines.append("    hide-ssid=yes \\")
    ...
    def _render_vpn(self, vpn) -> List[str]:
        ...
        if vpn.type == VPNType.WIREGUARD:
            ...
            lines.append("add name=wireguard1 \\")
            if vpn.listen_port:
